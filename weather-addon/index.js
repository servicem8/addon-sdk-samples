...
exports.handler = async (event) => {
    ...
    try {
        ...
        if (jobResponse.httpResponse.statusCode != 200) {
            return {
                eventResponse: wrapResponse('<p>An error occurred while fetching the details for job ' + strJobUUID + '. ' + JSON.stringify(jobResponse.httpResponse) + '</p>')
            };
        }
        ...
        if (bookingsResponse.httpResponse.statusCode != 200) {
            return {
                eventResponse: wrapResponse('<p>An error occurred while fetching scheduled bookings for Job ' + strJobUUID + '. ' + JSON.stringify(bookingsResponse.httpResponse) + '</p>')
            };
        }
        ...
        if (weatherResponse.httpResponse.statusCode != 200) {
            return {
                eventResponse: wrapResponse('<p>An error occurred while fetching scheduled bookings for Job ' + strJobUUID + '. ' + JSON.stringify(weatherResponse.httpResponse) + '</p>')
            };
        }
